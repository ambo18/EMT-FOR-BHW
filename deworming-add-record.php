...
<?php
    ...
    $query_deworming = "INSERT INTO tbl_deworming (p_name, dateofdeworming, typeofdeworming, remarks)
                VALUES (?, ?, ?, ?)";
    $stmt_deworming = $conn->prepare($query_deworming);
    $stmt_deworming->bind_param("ssss", $p_name, $dateofdeworming, $typeofdeworming, $remarks);
    $result_deworming = $stmt_deworming->execute();

    // Insert only p_name into tbl_operation_timbang
    $query_timbang = "INSERT INTO tbl_operation_timbang (p_name) VALUES (?)";
    $stmt_timbang = $conn->prepare($query_timbang);
    $stmt_timbang->bind_param("s", $p_name);
    ...
    if ($result_child && $result_deworming && $result_timbang && $result_vitamin) {
